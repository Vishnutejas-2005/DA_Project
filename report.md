...
## Methods
We ran Augmented Dickey-Fuller tests on each series and treated p-values below 0.05 as evidence of stationarity. The ADF results were: gdp_growth: p=0.304, npa_ratio: p=0.305, repo_rate: p=0.139. The series differenced once before estimation were: gdp_growth, npa_ratio, repo_rate. We then estimated a 3-variable VAR with lag order 1, selected by BIC from the information criteria table. Diagnostics show Durbin-Watson values of npa_ratio=1.85, repo_rate=1.63, gdp_growth=1.74; the system is stable = True; whiteness check: Portmanteau stat = 57.8661, p = 0.0119.
